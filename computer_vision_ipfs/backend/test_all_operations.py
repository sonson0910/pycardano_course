...
def test_all_operations():
    ...
    try:
        ...
        try:
            ...
            print(f"   ⚠️  Register failed (may need confirmation wait): {str(e)[:80]}...")
            tx_register = None
        ...
        print("RESULTS")
        print("="*80)
        print(f"[1] CREATE:   {tx_create[:32]}..." if tx_create else "[1] CREATE:   ❌ FAILED")
        print(f"[2] REGISTER: {tx_register[:32]}..." if tx_register else "[2] REGISTER: ⚠️  SKIPPED")
        print(f"[3] UPDATE:   {tx_update[:32]}..." if tx_update else "[3] UPDATE:   ⚠️  SKIPPED")
        print(f"[4] VERIFY:   {tx_verify[:32]}..." if tx_verify else "[4] VERIFY:   ⚠️  SKIPPED")
        print(f"[5] REVOKE:   {tx_revoke[:32]}..." if tx_revoke else "[5] REVOKE:   ⚠️  SKIPPED")
        
        if tx_create and tx_create != "709f1aee86f094d8609aef984904d7c60279625a266bbd09bad2a4afddb962a5":
